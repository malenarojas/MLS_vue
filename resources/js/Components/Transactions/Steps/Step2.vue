<template>
	<div class="rounded bg-indigo-50 p-4 shadow-md">
		<Toast />
		<h1 class="opacity-85 font-semibold text-lg">
		...
		<div class="mt-4 bg-white rounded p-4 border">
			<h2 class="opacity-85 font-semibold">
				...
				<span
					...
					"
					><i class="pi pi-home -translate-x-1 text-xs" />Comision
					Captador/Locator</span
				>
				<span
					v-if="
						!isEmptyObject(
					...
					"
					><i class="pi pi-dollar -translate-x-1 text-xs" />Comision
					Compra/Locatario</span
				>
			</h2>
			...
			<table class="w-full mt-4 opacity-75 text-sm">
				...
				<tbody class="text-center">
					<tr
						v-for="agente in store.getStepData(
							transactionId,
							2
						)"
						class="*:py-2 *:border-b"
					>
						...
						<td>
							<Select
								size="small"
								v-model="agente.commission_type_id"
								optionValue="id"
								optionLabel="name"
								class=""
						...
						<td>{{ agente.total_commission_percentage }} %</td>
						<td>
							<button
								v-if="agente.can_delete"
	...
	<!--new user external-->
	<Dialog
		v-model:visible="visibleNewOffice"
		modal
...
let visibleNewOffice = ref(false);
const formNuevo = ref({
	office_id: "",
	region_id: 120,
	user: {
...
const totalComision = computed(() => {
	...
	const stepData2 = store.getStepData(transactionId, 2);
	const soldPrice =
		store.getStepData(transactionId, 1)?.sold_price || 0;

	if (
		!isEmptyObject(stepData2) ||
		(Array.isArray(stepData2) && stepData2.length > 0)
	) {
		stepData2.forEach((agente) => {
...
const totalPorcentaje = computed(() => {
	...
	if (
		...
		stepData2.forEach((agente) => {
			const porcentaje =
				(agente.total_commission_amount * 100) / soldPrice || 0;
...
function borrarAgenteComision(agente) {
	let stepData = store.getStepData(transactionId, 2);
	const indiceBorrar = store
		.getStepData(transactionId, 2)
		.findIndex((item) => item == agente);
...
function selectAgente() {
	...
	store.getStepData(transactionId, 2).push({
