		...
		<div class="relative overflow-auto">
			<div class="overflow-x-auto">
				<table class="w-full order mb-2">
					...
					<tbody>
						<tr
							v-for="(payment, index) in store.getStepData(transactionId, 5)"
							:key="index"
...
const totalRecibido = computed(() => {
  let total = 0;
  const stepData5 = store.getStepData(transactionId, 5);

//   if (!isEmptyObject(stepData5) || (Array.isArray(stepData5) && stepData5.length > 0)) {
//     stepData5.forEach((payment) => {
//       total += parseFloat(payment.amount_received || 0);
//     });
//   }

  if (!isEmptyObject(stepData5)) {
		const stepDataArray = Array.isArray(stepData5) ? stepData5 : Object.values(stepData5);
...
//     });
//   }
	console.log('totalEsperado');
	console.log(parseFloat(total.toFixed(2)))
  return parseFloat(total.toFixed(2));
});

const agentesEnComisiones = computed(() => {
	const comisiones = store.getStepData(transactionId, 4);
	const agentes = [];
...
const totalRecibidoPendiente = computed(() => {
	return totalEsperado.value - totalRecibido.value;
}) ;
...
const borrarAgenteComision = (payment) => {
	store.saveStepData(transactionId, 5,store.getStepData(transactionId, 5).filter((item) => item != payment));
};
...
const selectAgente = () => {
	...
	store.getStepData(transactionId, 5).push({
